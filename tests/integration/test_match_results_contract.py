...
def test_failure_reasons_are_a_closed_set(bq):
    ...
    assert not (
        got
        & {
            "FALLBACK_REQUIRES_SCORING",
            "MULTIPLE_CANDIDATES_UNSCORED_EXACT",
...
def test_matcher_identity_still_cannot_read_the_labels():
    ...
    assert (
        one(
            c, f"SELECT COUNT(*) AS n " f"FROM `{PROJECT}.splitsheet_bronze.canonical_match_texts`"
        )["n"]
        > 0
    )
    with pytest.raises(Forbidden, match="(?i)access denied"):
        c.query(
            f"SELECT COUNT(*) FROM `{PROJECT}.splitsheet_eval.mapper_reference_labels`"
        ).result()
...
def test_sql_features_agree_with_the_python_implementation(bq):
    ...
    mismatches = []
    for s in sample:
        expected = F.features_for_pair(
            s["l_artist"], s["l_recording"], s["k_artist"], s["k_recording"]
        )
...
def _run_matcher(tmp_path, *extra: str):
    ...
    return subprocess.run(
        [
            sys.executable,
            str(repo / "src/matching/build_match_results.py"),
            "--norm-version",
            NORM_VERSION,
            "--blocking-version",
            "staged-1.0.0",
...
def test_rerun_is_idempotent_and_run_id_is_deterministic(bq, tmp_path):
    ...
    assert (
        report["match_run_id"] == before["run_id"]
