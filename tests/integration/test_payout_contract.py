...
def test_the_waterfall_reconciles_to_every_listen(bq):
    got = {
        r["attribution_status"]: int(r["listens"])
        for r in rows(
            bq,
            f"""
...
def test_the_five_metrics_are_not_conflated(bq):
    ...
    assert (
        r["rate_and_attributable"] + r["rate_but_risk_held"] + r["rate_but_defective"]
        == r["rate_resolved"]
    )
    assert (
        r["rate_and_attributable"] + r["own_but_risk_held"] + r["own_but_gap"]
...
def test_published_allocation_matches_the_python_implementation(bq):
    ...
    for group in sample:
        holders = [
            (h["rights_holder_id"], Decimal(h["holder_share_pct"])) for h in group["holders"]
        ]
        expected = {
            a.rights_holder_id: a.published
            for a in allocate(Decimal(group["gross_unrounded"]), holders)
        }
        published = {h["rights_holder_id"]: Decimal(h["holder_payout"]) for h in group["holders"]}
...
def test_two_publications_coexist_and_both_remain_queryable(bq):
    ...
    assert Decimal(published[PUBLISHED_RUN]["total_payout"]) == Decimal(
        published[REHEARSAL_RUN]["total_payout"]
    )
...
def test_a_mid_june_ownership_change_splits_the_money_between_two_holder_sets(bq):
    ...
    for recording, groups in by_recording.items():
        if len(groups) != 2:
            continue
        assert (
            groups[0]["holders"] != groups[1]["holders"]
