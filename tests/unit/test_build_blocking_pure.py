...
def test_a_different_run_id_does_not_satisfy_the_guard(monkeypatch, tmp_path):
    calls, outcome, _sqls = drive(
        monkeypatch, tmp_path, {**PUBLISHED, "run_id": "blk:0000000000000000"}
    )
...
def test_the_block_run_id_formula_is_unchanged():
    ...
    assert 'f"{args.norm_version}|{args.blocking_version}|{SNAPSHOT}|{EXPECTED_LISTENS}"' in body
    assert '"blk:" + hashlib.sha256(' in body
    recomputed = (
        "blk:"
        + hashlib.sha256(
            f"{NORM}|{BLOCKING}|{mod.SNAPSHOT}|{mod.EXPECTED_LISTENS}".encode()
        ).hexdigest()[:16]
    )
...
def test_an_unratified_run_id_does_not_inherit_the_proven_cardinality(monkeypatch, tmp_path):
    ...
    other_id = (
        "blk:"
        + hashlib.sha256(
            f"{NORM}|{other}|{mod.SNAPSHOT}|{mod.EXPECTED_LISTENS}".encode()
        ).hexdigest()[:16]
    )
    assert other_id not in mod.EXPECTED_CANDIDATES
    calls, outcome, _sqls = drive(
        monkeypatch, tmp_path, {**PUBLISHED, "run_id": other_id, "bver": other}, blocking=other
    )
    assert isinstance(outcome, ReachedHeavyWork)
    assert "stage_normalized" in calls
