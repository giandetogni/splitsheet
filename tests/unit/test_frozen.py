...
def test_the_frozen_normalization_copy_still_reproduces_the_baseline_version(frozen):
    ...
    frozen_copy = (
        MANIFEST_PATH.parents[1] / frozen["restatement"]["prior_normalization_frozen_copy"]
    )
...
def test_weights_and_thresholds_are_the_calibrated_ones(frozen):
    ...
    assert rules.weights == {
        "artist_unicode_exact": 0.1,
        "recording_unicode_exact": 1.0,
        "artist_token_similarity": 0.1,
        "recording_token_similarity": 1.0,
        "artist_string_similarity": 0.1,
...
def test_the_baseline_publication_identity_is_recorded(frozen):
    ...
    for key in (
        ...
        "baseline_attribution_run_id",
        "baseline_content_digest",
        "baseline_portfolio_paid",
        "baseline_row_count",
    ):
        assert r[key], key
...
def test_validation_partition_is_recorded_as_consumed(frozen):
    ...
    assert (
        frozen["evaluation"]["validation_consumed_under"] == frozen["pipeline"]["scoring_version"]
    )
...
def test_published_counts_are_internally_consistent(frozen):
    ...
    assert (
        p["structural_exact_unique"]
        + p["scored_exact_multiple"]
        + p["scored_fallback_unique"]
        + p["scored_fallback_multiple"]
    ) == p["matched"]
    unresolved = (
        p["below_threshold"]
        + p["ambiguous_tie"]
        + p["no_block_candidates"]
        + p["no_lookup_key_partial"]
        + p["no_lookup_key_empty"]
...
def test_the_published_run_identity_matches_the_identity_module(frozen):
    ...
    assert (
        ident["rejected_cohort_canonical_run_id"]
...
def test_the_published_attribution_run_id_derives_from_the_legacy_restatement_id(frozen):
    ...
    payload = (
        f"{r['payout_policy_version']}|{r['new_normalization_version']}"
        f"|{ident['legacy_run_id']}|RESTATED"
    )
    assert (
        "attr:" + hashlib.sha256(payload.encode()).hexdigest()[:16] == ident["attribution_run_id"]
    )
    canonical_payload = (
        f"{r['payout_policy_version']}|{r['new_normalization_version']}"
    ...
    assert (
        "attr:" + hashlib.sha256(canonical_payload.encode()).hexdigest()[:16]
