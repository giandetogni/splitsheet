...
def main() -> None:
    ...
    holders_mean = sum(n * w for n, w in zip(model.holders_per_recording,
                                             model.holders_per_recording_weights, strict=True))
    ...
    estimate = {
        ...
        "why": ("a recording held for match risk today becomes eligible under a future policy "
        ...
        "estimated_total_source_bytes": (split_rows * BYTES_PER_SPLIT_ROW
                                         + holders * BYTES_PER_HOLDER_ROW),
        ...
        "rejected_first_hypothesis": {
            ...
            "why_rejected": ("1.6M holders for 2.47M recordings models a catalogue where "
    ...
    est_tib = (estimate["estimated_total_source_bytes"]
               + estimate["estimated_temporal_join_input_bytes"] * 4) / 1024**4
    ...
    estimate["cost_caveat"] = ("list-price equivalent of processing consumption; actual "
    ...
    report = {
        ...
        "policy": {
            ...
            "basis": ("validation-partition disagreement 3.2138% for SCORED_FALLBACK_UNIQUE "
                      "against 0.0055% for STRUCTURAL_EXACT_UNIQUE, and no approved business "
                      "risk policy for paying on it"),
            "not_validated_by": ("the consumed validation partition; this is a business rule "
    ...
    print(f"  payout-eligible matches     {u['payout_eligible_matches']:>12,}"
          f"  ({100 * u['payout_eligible_matches'] / u['listens']:.4f}% of listens)")
    print(f"  risk-held matches           {u['risk_held_matches']:>12,}"
          f"  ({100 * u['risk_held_matches'] / u['listens']:.4f}%)")
    print(f"  distinct recordings: matched {u['distinct_matched_recordings']:>11,}  "
    ...
    print(f"    materialisation + queries {e['estimated_materialisation_and_query_tib']} TiB "
