...
def main() -> None:
    ...
    print(
        f"restating {PRIOR_PUBLICATION_ID} ({prior_run}) -> {NEW_PUBLICATION_ID} ({new_run})",
        flush=True,
    )
    print(f"  payout policy UNCHANGED: {policy.version}", flush=True)
    print(
        f"  normalization {policy.scoring_version} scoring UNCHANGED; normalization moved to "
    ...
    def dbt(cmd: list[str], dbt_vars: dict, label: str) -> dict:
        proc = subprocess.run(
            [str(DBT_BIN), *cmd, "--vars", json.dumps(dbt_vars)],
            cwd=DBT_DIR,
        ...
        print(
            f"  dbt {label:<44} exit={proc.returncode}  "
            f"{done[-1].strip()[-60:] if done else ''}",
            flush=True,
        )
    ...
    """,
        "read the frozen baseline from the registry",
    )[0]
    ...
    if digest_before["content_digest"] != baseline["content_digest"]:
        raise SystemExit(
            f"v1 has already changed: registry {baseline['content_digest']} vs "
    ...
    """,
        "verify the restated match table",
    )[0]
    ...
    if restated["normalization_version"] != args.new_normalization_version:
        raise SystemExit(
            f"restated matches carry {restated['normalization_version']}, expected "
            f"{args.new_normalization_version}"
        )
    if restated["scoring_version"] != policy.scoring_version:
        raise SystemExit(
            "the restated matches were scored under a different scoring version; a "
    ...
    runs.append(
        dbt(
            ["build", "--select", "stg_silver__listen_matches+ int_attributable_streams+"],
            dbt_vars,
            "recompute financial layer over restated matches",
        )
    )
    ...
    immutable = {
        ...
        "portfolio_paid_unchanged": str(v1_stamp["portfolio_paid"])
        == str(baseline["portfolio_paid"]),
    }
    ...
    """,
        "pointer moved to v2, v1 still queryable",
    )[0]
    ...
    report = {
        ...
        "new": {
            ...
            "recordings": int(v2["recordings"]),
            "holders": int(v2["holders"]),
        },
        "unchanged_inputs": {
            "payout_policy_version": policy.version,
        ...
        "reconciliation": {
            k: (str(v) if v is not None else None) for k, v in reconciliation.items()
        },
        "dbt_runs": runs,
        "cost": {
    ...
    print(
        f"  V2: {int(v2['row_count']):,} holder rows, {v2['portfolio_paid']} USD, "
        f"digest {v2_digest['content_digest'][:24]}..."
    )
    print(
        f"  pointer -> {pointer_state['current_run']}; current view "
    ...
    print(
        f"    listens         {r['listens_total']:,}  none disappeared: "
        f"{r['no_listen_disappeared']}"
    )
    print(
        f"    newly matched   {r['newly_matched']:,}   still unmatched {r['still_unmatched']:,}   "
        f"matches lost {r['matches_lost']}   newly ambiguous {r['newly_ambiguous']:,}"
    )
    print(
        f"    holders added   {r['holders_added']:,}   removed {r['holders_removed']:,}   "
