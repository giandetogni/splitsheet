...
def main() -> None:
    ...
    disposition = q(f"""
        ...
        GROUP BY 1, 2, 3
    """, "financial disposition after") if matched_mbid else []
    ...
    """, "money for this recording, per publication") if matched_mbid else []
    ...
    """, "restatement delta for this recording") if matched_mbid else []

    v1_state = by_side.get("v1", {})
    v2_state = by_side.get("v2", {})
    resolved_safely = (v2_state.get("match_status") == "MATCHED"
                       and v2_state.get("match_method") == "STRUCTURAL_EXACT_UNIQUE")

    report = {
        "artifact": "concrete_case",
        "case": {"artist": ARTIST, "recording": RECORDING,
        ...
        "after": {
            ...
            "transliterated_representation": {
                ...
                "note": ("the stored Unicode value is unchanged and still Korean; only the LOOKUP "
        ...
        "honest_verdict": (
            ...
            if resolved_safely else
            "the case did NOT resolve into a safe unique match; reported as measured rather than "
            "forced to work"),
    ...
    print(f"    normalized unicode  {before_norm.artist_normalized_unicode} / "
          f"{before_norm.recording_normalized_unicode}")
    print(f"    lookup key          {before_norm.lookup_exact!r}  "
          f"({before_norm.exact_key_status.value})")
    print(f"    listens             {int(v1_state.get('listens', 0)):,}")
    print(f"    match               {v1_state.get('match_status')} / "
          f"{v1_state.get('failure_reason')}")
    print("    payout              none: never matched")
    print(f"\n  AFTER ({v2.version})")
    print(f"    transliterated key  {after_norm.lookup_exact!r}  "
          f"({after_norm.exact_key_status.value})")
    print(f"    normalized unicode  {after_norm.recording_normalized_unicode}  "
    ...
    print(f"    match               {v2_state.get('match_status')} / "
