...
def main() -> None:
    ...
    changed = [t for t in transitions
    ...
    collisions = q(f"""
        ...
        WHERE {period('r')} AND {period('v')} AND r.recording_cohort
          AND v.match_status != 'MATCHED'
    """, "candidate collision rate (unsupervised)")[0]
    ...
    report = {
        ...
        "unsupervised": {
            ...
            "candidate_collision": {k: (float(v) if v is not None else None)
