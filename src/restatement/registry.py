...
def _cohort_row(key: str, cohort: CohortDefinition, inputs: RestatementInputs,
                legacy: str) -> dict:
...
def registry_rows(cohorts: dict[str, CohortDefinition] | None = None,
                  legacy_runs: list[LegacyRun] | None = None) -> list[dict]:
    ...
    rows = [_cohort_row(key, cohort, inputs_for_published_restatement(registry, key), legacy)
...
def registry_model_sql(cohorts: dict[str, CohortDefinition] | None = None,
                       legacy_runs: list[LegacyRun] | None = None) -> str:
...
def main() -> None:
    ...
    for row in registry_rows():
        print(f"  {row['registry_key']:<42} {row['run_type']:<28} "
