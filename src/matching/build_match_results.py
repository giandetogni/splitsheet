...
def main() -> None:
    ...
    run_id = "match:" + hashlib.sha256(
        ...
        f"{rules.version}|{F.FEATURE_VERSION}".encode()).hexdigest()[:16]
    ...
    checks = {
        ...
        "fallback_unique_accepted_above_threshold":
            int(v["fallback_accepted_below_threshold"]) == 0,
        "scored_acceptance_respects_threshold_and_margin":
            int(v["exact_multi_accepted_below_policy"]) == 0
    ...
    for d in dist:
        print(f"  {d['match_method']:<25} {d['match_status']:<11} "
